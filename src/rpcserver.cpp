...
int64_t AmountFromValue(const Value& value)
{
    ...
    int64_t nAmount = roundint64(dAmount * Params().CoinMultiplier());
    if (!Params().InMoneyRange(nAmount))
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount");
    return nAmount;
}
...
{
    return (double)amount / (double)Params().CoinMultiplier();
}
