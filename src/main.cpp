...
bool GenerateCoinbaseCommitment(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    int commitpos = -1;
    ...
    if (IsWitnessEnabled(block, pindexPrev, consensusParams)) {
        if (block.vtx[0].wit.vtxinwit.size() != 1 || block.vtx[0].wit.vtxinwit[0].scriptWitness.stack.size() != 1 || block.vtx[0].wit.vtxinwit[0].scriptWitness.stack[0].size() != 32) {
            ...
            block.vtx[0].wit.vtxinwit[0].scriptWitness.stack[0] = ret;
        }
        if (commitpos == -1) {
