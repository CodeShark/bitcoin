...
bool CScriptCheck::operator()() {
    ...
    const CScriptWitness *witness = (nIn < ptxTo->vwit.size()) ? &ptxTo->vwit[nIn].scriptWitness : NULL;
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion >= 5 && IsSuperMajority(5, pindexPrev, consensusParams.nMajorityEnforceBlockUpgrade, consensusParams)) {
        ...
        if (block.vtx[0].vwit.size() == 0 || block.vtx[0].vwit[0].scriptWitness.stack.size() != 1) {
             return state.DoS(100, error("%s : invalid witness merkle path length", __func__), REJECT_INVALID, "bad-witness-merkle-len", true);
        }
        if (!CheckCoinbaseCommitment(block.vtx[0].vin[0].scriptSig, hashWitness, block.vtx[0].vwit[0].scriptWitness.stack[0], vTypeWitnessCommitment)) {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    if (inv.type == MSG_WITNESS_BLOCK)
                        pfrom->PushMessageWithFlag(SERIALIZE_TRANSACTION_WITNESS, "block", block);
            ...
            else if (inv.type == MSG_TX || inv.type == MSG_WITNESS_TX)
            {
                ...
                bool pushed = false;
                {
                    ...
                    if (mi != mapRelayTx.end()) {
                        pfrom->PushMessageWithFlag(inv.type == MSG_WITNESS_TX ? SERIALIZE_TRANSACTION_WITNESS : 0, "tx", (*mi).second);
                        pushed = true;
                    }
                }
                if (!pushed && inv.type == MSG_TX) {
                    ...
                    if (mempool.lookup(inv.hash, tx)) {
                        pfrom->PushMessageWithFlag(inv.type == MSG_WITNESS_TX ? SERIALIZE_TRANSACTION_WITNESS : 0, "tx", tx);
                        pushed = true;
                    }
                }
                if (!pushed) {
...
{
    return (pindex->nVersion >= 5 && IsSuperMajority(5, pindex->pprev, consensusParams.nMajorityEnforceBlockUpgrade, consensusParams));
}
