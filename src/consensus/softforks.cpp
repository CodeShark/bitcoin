...
VersionStatus Consensus::SoftForks::CheckVersion(const CBlockIndex& blockIndex, const Consensus::Params& consensusParams, CBlockIndex* pindexPrev)
{
    ...
    if (UsesVersionBits(blockIndex.nVersion))
        return GetBlockRuleIndex().AreVersionBitsRecognized(&blockIndex, pindexPrev) ? VALID : UNRECOGNIZED;
...
bool Consensus::SoftForks::UseRule(int rule, const CBlockIndex& blockIndex, const Consensus::Params& consensusParams, CBlockIndex* pindexPrev)
{
    if (!pindexPrev)
        pindexPrev = blockIndex.pprev;
    ...
    default:
        ...
        return (Consensus::VersionBits::GetBlockRuleIndex().GetRuleState(rule, &blockIndex) == Consensus::VersionBits::ACTIVE);
    }
